class ItemConfigure {
	...
	show_range_input_if_applicable(fieldname) {
		...
		if (changed_value && changed_value.includes(" to ")) {
			...
			if (!numbers.some((n) => isNaN(n))) {
				...
				const parent = $('<div class="range-selector">').insertBefore(
					changed_field.$input_wrapper.find(".help-box")
				);
	...
	show_remaining_optional_attributes() {
		...
		const is_optional_attribute = (df) => {
			const optional_attributes = this.attribute_data.filter((a) => a.optional).map((a) => a.attribute);
	...
	get_html_for_item_found({
		...
		const one_item =
			exact_match.length === 1 ? exact_match[0] : filtered_items_count === 1 ? filtered_items[0] : "";
		...
		if (
			!product_info?.allow_items_not_in_stock &&
			available_qty === 0 &&
			product_info &&
			product_info?.is_stock_item
		) {
			item_add_to_cart = "";
		}
	...
	btn_add_to_cart(e) {
		...
		const additional_notes = Object.keys(this.range_values || {})
			.map((attribute) => {
